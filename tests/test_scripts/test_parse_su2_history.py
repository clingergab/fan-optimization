...
#
# Removed 2026-05-14: the test piped parse_su2_history → run_spike_0_6c_2
# (the now-retired NACA 0012 benchmark analyzer) → sub_2.PASS marker. With
# 0.6c.2 deferred to Phase 5 the analyzer no longer exists; the test
# referenced run_spike_0_6c_2 directly and would ImportError on collection.
#
# The parse step itself remains exercised by the unit tests above
# (cycles_from_rows, _read_history, _per_outer_iter, etc.). A new
# end-to-end test will land when Phase 5 ships its cross-solver
# analyzer (which will replace the deleted Phase-0 analyzer in the
# parse → analyze → marker pipeline).
