...
**Revision:** R2 -- Addressing Round 2 review feedback
...
**Minimum viable project --- Option A (no SU2, fastest):** For a truly beginner-accessible path that avoids the steepest learning curve component (SU2), skip both the ML surrogate and formal shape optimization:

1. Design baseline fan in FreeCAD, print it, measure airflow.
2. Run CFD on 5-10 hand-designed shape variants using [SimScale](https://www.simscale.com/) (browser-based, no installation, free tier available). Evaluate the pressure distribution on each, pick the best.
3. Run TO on the best shape using FreeCAD/BESO.
4. Print and validate.

This eliminates SU2, BoTorch, and formal ASO entirely, replacing them with manual design-of-experiments using SimScale's beginner-friendly browser interface. Estimated timeline: 3-4 weeks. The trade-off is that you explore far fewer shapes and miss the adjoint-based gradient information that makes SU2 powerful.

**Minimum viable project --- Option B (with SU2, no surrogate):** Skip the ML surrogate layer but use SU2's built-in adjoint-based ASO directly (no surrogate needed --- SU2 computes exact gradients via the adjoint method). This eliminates BoTorch/GP setup and reduces the project to: baseline CFD -> adjoint ASO -> TO -> print. Estimated timeline: 4-6 weeks for someone with basic Python and CAD skills. **Note:** SU2 is the hardest tool in this stack (command-line only, complex config files, external mesh generation). Budget 2-4 weeks for learning SU2 alone if you have no prior CFD experience.
...
1. **SU2 objective function replaced (Section 9.3) [AGREE with Issue 2.1]:** Removed `SURFACE_PRESSURE_DROP`, which is designed for internal flow (inlet/outlet pairs) and does not apply to external aerodynamics. Replaced with three-tier approach: (a) LIFT-based objective as simplest fallback, (b) `SURFACE_TOTAL_PRESSURE` on a downstream analysis plane as recommended approach, (c) custom Python wrapper computing net momentum flux for highest fidelity. Added explicit explanation of why SURFACE_PRESSURE_DROP was wrong. Added Python code example for custom objective.

2. **SU2 unsteady solver switched to compressible (Section 9.3) [AGREE with Issue 2.2]:** Replaced `INC_NAVIER_STOKES` with `NAVIER_STOKES` + `LOW_MACH_PREC= YES` for the unsteady pitching configuration. Added documentation of GitHub Issue #193 and the fundamental point that all SU2 unsteady pitching tutorials use the compressible formulation. Retained incompressible solver for the steady-state proxy only.

3. **PETG material properties corrected to FDM values (Sections 6.1, 9.2) [AGREE with Issue 2.3]:** Changed E from 2100 MPa (bulk datasheet) to 1300 MPa (FDM XY-direction) and 1000 MPa (FDM Z-direction). Added FDM-specific rows to the materials comparison table. Added warning about infill density scaling. Added recommendation to print and test tensile specimens for calibration.

4. **Steady-state proxy limitations quantified (Section 9.3.1) [PARTIALLY AGREE with Issue 2.4]:** Added new subsection computing the reduced frequency (k ~ 0.6), listing the specific physical effects absent from steady-state (added mass, wake interaction, dynamic stall), and establishing a mandatory 3-design validation protocol. Maintained the steady-state proxy as a valid beginner starting point but with explicit caveats and a required validation step.

5. **Mesh generation difficulty addressed (Section 9.3) [AGREE with Issue 4.1]:** Added new subsection on the challenges of meshing thin fan blades (100:1 aspect ratio, boundary layer resolution, trailing edge singularity, downstream analysis plane setup). Included practical Gmsh tips and time estimate (1-2 weeks for beginners).

6. **Convergence diagnosis guidance added (Section 8.3.2) [AGREE with Issue 4.2]:** Added new subsection with diagnostic tables for both TO and BO convergence problems. Covers oscillatory convergence, stalling, checkerboard patterns, intermediate densities, BESO member deletion, GP numerical issues, and budget exhaustion.

7. **TopOpt alpha status flagged (Section 9.1) [AGREE with Issue 2.5]:** Added explicit warning about pre-release status (v0.0.1-alpha.1). Added DTU TopOpt Python codes as more mature educational alternative.

8. **Minimum viable project restructured (Section 8) [PARTIALLY AGREE with Issue 2.6]:** Added "Option A" MVP using SimScale (browser-based, no SU2) for a truly beginner-accessible path. Retained the SU2-based MVP as "Option B" with explicit warning about the SU2 learning curve.
...
*This report was generated through comprehensive web research on 2026-03-22 and revised through two rounds of expert review. R2 revisions specifically addressed SU2 solver configuration issues (objective function and incompressible/compressible solver selection), FDM material property accuracy, steady-state proxy limitations, mesh generation challenges, and convergence diagnostics. Software versions and pricing are subject to change. Always verify current availability and compatibility before committing to a toolchain.*
