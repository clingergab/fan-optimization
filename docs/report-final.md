...
> **⚠ Blade geometry locks superseded by [ADR-0005](adr/0005-trapezoid-blade-geometry-redesign.md) (2026-07-29).**
> The geometry/dimension locks below (200 mm pie-slice blade, rib band `[0.020, 0.185]`,
> `blade_count {8,10,12}`, `< 100 g`, panel-pivot `width = r·INTER_BLADE_ANGLE`) are the OLD
> geometry. ADR-0005 is authoritative for the current trapezoid blade (12 blades fixed, 220 mm,
> one-part root, 300 g cap). Non-geometry locks (CFD numerics intent, C11 sign, etc.) still hold.
...
> **⚠ Geometry superseded by [ADR-0005](adr/0005-trapezoid-blade-geometry-redesign.md) (2026-07-29).**
> The dimensions and pie-slice/panel-pivot architecture in this section (200 mm blade,
> `width = r·INTER_BLADE_ANGLE`, rib band `[0.020, 0.185]`, boss at `pivot_center_x`, `< 100 g`
> mass cap) are the OLD geometry. The current locked geometry is the **trapezoid blade** in
> ADR-0005: 12 blades (fixed), 220 mm length, root = 12 mm at the pivot centre (blade+boss one
> part), tip ≈ 51 mm, panel base 3 mm, two modes (panel+ribs / single-thickness no-rib), 300 g
> mass cap. Read ADR-0005 as authoritative where it conflicts with the text below.
...
> **⚠ Geometry inputs superseded by [ADR-0005](adr/0005-trapezoid-blade-geometry-redesign.md) (2026-07-29).**
> Equations here that plug in `L_blade = 0.20 m`, `L_wrist_to_tip = 0.25 m`, the 165 mm rib, or
> the pie-slice tangential width use the OLD geometry — re-derive with the ADR-0005 trapezoid
> (220 mm length → `L_wrist_to_tip = 0.27 m`, higher tip speed → CFD `Re`/`Mach` re-derived per
> ADR-0005). The rib-only TO premise also changes: mode B has no ribs. The physics/formulations
> (plate-bending, SIMP, the force models) still hold; only the geometry constants move.
...
| 0.6d | (H10 supplement; 2026-05-14 addition, redesigned 2026-05-15) | Tier-1 added-mass frequency-consistency gate (0.6d.2 two-frequency check is the sole gate; 0.6d.1 + 0.6d.3 advisory) | 0.6c.1 (cfg sanity must clear first); 2× short 2D-thin-plate SU2 runs (ω₁, ω₂) |
...
- **Sub-spike 0.6c.2 — Published-benchmark validation: DEFERRED TO PHASE 5 (2026-05-14 decision).** The original Phase-0 framing — run a published oscillating airfoil case through the working Tier 1 cfg setup, reference case NACA 0012 pitching about quarter-chord at `k_reduced ≈ 0.5-0.6`, low Reynolds (~30k-50k) — was demonstrated unsound by the 2026-05-14 regime diagnostic (`scripts/diagnose_su2_pitching_regime.py`): the production Tier-1 numerics simulate body-in-still-air added-mass / quadratic-drag physics correctly but cannot be validated against published *wind-tunnel-frame* references (different regime; CL bias ratio 2.234, phase lag +88.2°, CL at 2× the prescribed pitching frequency — all consistent with body-in-still-air, all inconsistent with the wind-tunnel reference data). The full evidence trail is in `docs/phase_logs/spike_0_6c.md` Note 1; the V1/V2 decision is in `docs/phase_logs/phase_0_signoff.md` Note 1. The published-reference validation moves to **Phase 5 step 62.5** as a body-in-still-air-frame benchmark with full force-trace data from the insect-flight / Morison-equation literature, run through SU2 + PyFR + OpenFOAM (3-solver cross-check). To compensate for the lost Phase-0 absolute-accuracy evidence, **Spike 0.6d** (below) lands a normalization-invariant added-mass frequency-consistency gate. Phase 4 launch is no longer gated on 0.6c.2 (see updated fail-action below); **0.6c.1 + 0.6d.2** are the V1 gate set (0.6d.1 + 0.6d.3 advisory, post-2026-05-15 redesign).
- **Fail action (V1 gate set, post-2026-05-15 redesign):** Phase 4 launch is **gated on Spike 0.6c.1 PASS AND Spike 0.6d.2 PASS** (0.6c.2 deferred to Phase 5 per Note above; 0.6d.1 + 0.6d.3 are advisory, NOT gating — see §Phase 0 Spike 0.6d + `docs/phase_logs/phase_0_signoff.md` Note 3). `scripts/launch_phase4.py` refuses to create the `phase4-launch` tag unless both `data/spike_0_6c/PASS` AND `data/spike_0_6d/PASS` markers are present. (Marker path `phase0/spike_0_6c/PASS` in earlier plan text resolves to `data/spike_0_6c/PASS` per the as-built repository layout.) If any gating sub-spike fails, investigate the production Tier-1 cfg (low-Mach preconditioner coefficients, dual-time inner-iter count, dt convergence, mesh quality) before Phase 4 launches; do NOT silently proceed.

**Spike 0.6d: Tier-1 added-mass frequency-consistency gate (2026-05-14 addition; redesigned 2026-05-15; H10 lock supplement — gates Phase 4 launch alongside 0.6c.1)**

- Motivation: the 2026-05-14 Spike 0.6c regime diagnostic confirmed the production Tier-1 cfg simulates body-in-still-air physics but cannot be validated against published wind-tunnel references; 0.6c.2 was deferred to Phase 5. Spike 0.6d provides the independent Phase-0 quantitative check that prevents Phase 4 from burning ~1300 GPU-hours on a Tier-1 objective we can't trust. **Redesign (2026-05-15):** the first live run exposed that the original 0.6d.1 magnitude check conflated SU2's `q_ref=1` nondimensional output with a dimensional Newton envelope, and its symmetry criterion is ill-posed for the fan's net-work regime (a working fan produces net force by design). The gate was rebuilt to rest solely on a **normalization-invariant, parameter-free falsification test** (0.6d.2 below). Full decision record: `docs/phase_logs/phase_0_signoff.md` Note 3.
- **Sub-spike 0.6d.2 — 2D thin-plate added-mass frequency-consistency** (GATING — the sole Phase-4 gate): run the 2D thin-plate at TWO pitching frequencies (ω₁, ω₂) with the same plate / pivot / θ_max, Fourier-project each moment-coefficient trace onto the added-mass (sin φ) basis, and recover `I_a = a_sin/(ω²·θ_max)`. `I_a = πρb⁴(1/8+a²)` is a pure geometric/fluid constant with NO frequency dependence — if SU2's MACH=1e-9 + low-Mach-prec unsteady solver is physically faithful, the recovered `I_a` MUST agree between ω₁ and ω₂. **Pass criterion:** `|I_a(ω₁) − I_a(ω₂)| / mean < 0.25`. This is normalization-invariant (SU2-vs-SU2; the FREESTREAM_PRESS_EQ_ONE `q_ref` is a fixed constant identical to both runs, cancels in the ratio) and parameter-free. A Sedov/Newman closed-form magnitude comparison is also computed but is **advisory** (its absolute scale needs SU2's exact reference-state handling, which is Phase 5 step 62.5's job). **Cost:** ~2 short Colab CPU SU2 runs on a tiny 2D mesh (~1–2 h each). **Implementation:** `configs/su2/thin_plate_2d_pitching.cfg.j2` + `render_thin_plate_2d_pitching_cfg` + `recover_added_mass_projection` / `check_added_mass_freq_consistency` in `src/fanopt/cfd/spike_0_6d.py` + `scripts/run_spike_0_6d_2.py`.
- **Sub-spike 0.6d.1 — Symmetry + dimensional-force sanity** (ADVISORY, NOT gating — demoted 2026-05-15): runs the symmetry + dimensional-envelope checks on the existing Spike 0.6c Cell 8 history.csv. Recorded for Phase 5 (raw material for the step-62.5 cross-solver work) but does **not** gate: its magnitude check is confounded by the `q_ref=1` nondimensionalisation and its near-zero-cycle-mean criterion is wrong for a fan that does net work. **Implementation:** `check_symmetry_dimensional` in `src/fanopt/cfd/spike_0_6d.py` + `scripts/run_spike_0_6d_1.py`.
- **Sub-spike 0.6d.3 — SU2 incompressible-mode cross-check** (ADVISORY, NOT gating): duplicate the production cfg with `SOLVER= INC_NAVIER_STOKES`. Recorded for Phase 5; the cross-codebase answer is Phase 5 step 62.5 (OpenFOAM `pimpleFoam`).
- **Aggregate marker file:** `scripts/run_spike_0_6d.py` writes `data/spike_0_6d/PASS` iff **sub_2's frequency-consistency check passes** (`overall_passed = sub_06d_2.freq_consistency_passed` ONLY). sub_1 + sub_3 advisory results are recorded in the result JSON (they feed Phase 5 step 62.5) but do NOT affect the marker.
...
**Spikes still required for V1:** 0.0 (done), 0.1 (done), 0.4, 0.6, 0.6a/0.6b (gates), **0.6c.1 (non-negotiable — Phase 4 launch gate; 0.6c.2 deferred to Phase 5 per 2026-05-14 decision, see §Phase 0 Spike 0.6c notes and Phase 5 step 62.5)**, **0.6d.2 (2026-05-14 addition, redesigned 2026-05-15 — Tier-1 added-mass frequency-consistency; the sole Spike-0.6d Phase-4 gate alongside 0.6c.1; 0.6d.1 + 0.6d.3 advisory)**, 0.7a, 0.7b. The 15-30% gain target in §0 row 35 is **suspended for V1**; V1 reports sim-vs-sim relative gain plus the operator's qualitative feel comparison. The numerical gain target re-enters in V2 once a quantitative baseline exists.
...
9. **V1.5 — staggered AO↔TO co-optimization (operator direction 2026-07-18).** V1 runs AO (Phase 4 BO) and TO (Phase 2 SIMP) sequentially + decoupled; V1.5 closes the loop with a staggered (block Gauss-Seidel) iteration — pass aero pressures → TO, updated rib mass/stiffness → AO, iterate to convergence. **Purely computational — needs none of the deferred V2 measurement hardware.** Reuses both existing solvers; the only new code is the coupling orchestration. Its products double as the ground-truth coupled baseline + the seed dataset for item 10. The panel-compliance→aero channel needs a static-deflection step (relaxes the V1 "No FSI" lock). Item 8 (Winkler BC) is the cheap one-way alternative; V1.5 is the full loop. **Full spec + acceptance in `docs/V2_backlog.md` § "V1.5 — Staggered AO↔TO co-optimization".**

10. **ML-driven TO + AO — escape the parameterization ceiling (operator direction 2026-07-18).** Pursue ML-based topology + aero optimization (surrogate → generative → neural-operator MDO), not just deterministic SIMP/GP-BO. **North star:** the §10 / §7 35–40-var codec is a *compute-driven expert simplification*, not a fundamental limit — BO can only find the best fan *expressible in that hand-picked basis*. The deepest payoff of ML + GPU is removing that ceiling (higher-dim → free-form neural-implicit / voxel-TO → generative latent spaces). Distinguishes basis simplifications (escape) from product-defining locks — panel-pivot architecture, m < 100 g, PETG printability, click, kinematics (keep). Physics stays the ground-truth verifier (surrogate proposes; SU2/FEA + §59.5 dispose). Highest-ROI first target = the aero surrogate (CFD is the ~85 min/eval bottleneck). **Full staged path in `docs/V2_backlog.md` § "ML-driven TO + AO".**
...
| **Spike 0.6d — Tier-1 added-mass frequency-consistency (2026-05-14 addition; redesigned 2026-05-15; H10 supplement)** | Colab Pro CPU | **~2–4 hours total** (2× short 2D-thin-plate SU2 runs at ω₁, ω₂ for the gating 0.6d.2; 0.6d.1 reuses existing Cell-8 history.csv at ~0 compute; 0.6d.3 advisory optional) | Phase 0; gates Phase 4 launch alongside 0.6c.1. NOT booked against the 1000-h Phase 4 stop rule. |
