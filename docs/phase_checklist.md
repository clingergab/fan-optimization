...
- [x] **3D SU2 verification pipeline** (landed 2026-07-03, parallel with the Phase-4 campaign): `fanopt.cfd.mesh` (3D external-flow volume mesher: STEP body → box-minus-body → wall-refined tet mesh) + `fanopt.cfd.phase5` + `scripts/run_phase5_verify.py`. Takes the top-k diverse Pareto designs → single V-unit blade → 3D unsteady SU2 → canonical J_fan → Kendall-τ check that the 2D-slice ranking holds at 3D fidelity (adds the finite-span/tip physics the slice omits). **Single blade, not the full fan** — the deployed fan's click-mating faces are non-manifold and won't tet-mesh. **Found + fixed a real bug:** the `fan3d_steady` cfg used SU2-8.0.1-invalid `FREESTREAM_OPTION=REYNOLDS` / `FREESTREAM_DIRECTION` (never run before) → fixed to `TEMPERATURE_FS` + AOA/sideslip + low-Mach prec (mirrors the 2D `slice_steady` fix). Both 3D cfgs validated with real SU2 (steady CD=0.177 converged; unsteady Exit Success). Full unsteady runs are a Colab job; geometry+mesh+cfg are local. 100% cov on new modules.
